...
#extract CVE core fields
    def extract_cve_core_fields(vuln: dict) -> dict:
       ...
       #CWEIDS
        ...
        metrics = cve.get("metrics", {})
        if metrics.get("cvssMetricV31"):
            base_score = metrics["cvssMetricV31"][0]["cvssData"].get("baseScore")
        elif metrics.get("cvssMetricV40"):
            base_score = metrics["cvssMetricV40"][0]["cvssData"].get("baseScore")

        #attack vector and attack complexity
        attack_complexity = None
        attack_vector = None
        if metrics.get("cvssMetricV31"):
            attack_complexity = metrics["cvssMetricV31"][0]["cvssData"].get("attackComplexity")
            attack_vector = metrics["cvssMetricV31"][0]["cvssData"].get("attackVector")
        elif metrics.get("cvssMetricV40"):
            attack_complexity = metrics["cvssMetricV40"][0]["cvssData"].get("attackComplexity")
            attack_vector = metrics["cvssMetricV40"][0]["cvssData"].get("attackVector") 
...
def extract_gsa_fix_fields(advisory: dict) -> dict:
    """
    Extract remediation-relevant fields from a GSA advisory.
    
    Handles both new flat structure (from fetch_gsa_data) and old nested structure.
    New structure fields: cve_id, ghsa_id, summary, description, severity, 
                        vulnerable_version_range, first_patched_version, cwes, package
    """
    
    # Check if this is the new flat structure or old nested structure
    if "ghsa_id" in advisory:
        # New flat structure from fetch_gsa_data()
        # Extract and format CWEs from the raw cwes list
        cwes_formatted = []
        ...
        return {
            ...
            "vulnerable_versions": advisory.get("vulnerable_version_range", ""),
            "fixed_version": advisory.get("first_patched_version", ""),
            "references": advisory.get("references", []),
            "cwes": cwes_formatted
        }
    
    # Legacy/old nested structure handling (fallback)
    # -------------------------
    # 1. Identifiers
    # -------------------------
    cve_id = None
